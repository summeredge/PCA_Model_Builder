...
<script>
const state = { fileId:null, runId:null, exploratoryRunId:null, inspection:null, clustering:null, exploration:null, explorationPcMode:"cluster", preferredRegion:null, preferredRegionDrawing:false, preferredRegionRequest:0, preferredRegionUpdateSeq:0, performance:null, training:null, trend:null, preprocessingPreview:null, preprocessingPreviewTag:null, registry:{}, quality:null, qualityStatus:"unchecked", qualityError:"", selectedTag:null, selectedModelTags:new Set(), importPreview:null, excludedTags:[], excludedWindows:[], showProblems:false, candidateWindows:[], trainingWindows:[], trainingWindowSummary:[], validationWindows:[] };
...
function renderExplorationPcModeControls(data) {
  const cluster=el("explorationPcModeCluster"), performance=el("explorationPcModePerformance"); if(!cluster||!performance) return;
  const targetRange=data?.performance_config?.direction==="target_range"; if(!targetRange) state.explorationPcMode="cluster";
  cluster.disabled=!data; performance.hidden=!targetRange; performance.disabled=!targetRange;
  cluster.classList.toggle("active",state.explorationPcMode==="cluster"); performance.classList.toggle("active",targetRange&&state.explorationPcMode==="performance");
}
function renderExplorationPcChart(data) {
  renderExplorationPcModeControls(data);
  ...
  const targetRange=data.performance_config?.direction==="target_range";
  const performanceMode=targetRange&&state.explorationPcMode==="performance";
  const points=finiteRows.map(row=>{const color=explorationClusterColor(row.cluster_id);const faded=performanceMode&&row.performance_target_met!==true;return `<circle class="exploration-pc-point" cx="${x(Number(row.pc1)).toFixed(2)}" cy="${y(Number(row.pc2)).toFixed(2)}" r="3" fill="${color}" fill-opacity="${faded?".25":".75"}"></circle>`;}).join("");
  const targetHalos=performanceMode?finiteRows.filter(row=>row.performance_target_met===true).map(row=>`<circle class="exploration-pc-target-halo" cx="${x(Number(row.pc1)).toFixed(2)}" cy="${y(Number(row.pc2)).toFixed(2)}" r="6" fill="none" stroke="${explorationClusterColor(row.cluster_id)}" stroke-width="2" vector-effect="non-scaling-stroke"></circle>`).join(""):"";
  ...
  const targetLegend=performanceMode?`<text x="${pad}" y="${height-8}" fill="#111827" font-size="10">◎ 外圈：性能达标　淡化点：未达标或无有效性能值</text>`:"";
  ...
  container.innerHTML=`<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Cluster PC1 PC2 散点图">${legend}${targetLegend}<line x1="${pad}" x2="${width-pad}" y1="${height/2}" y2="${height/2}" stroke="#d7dee8"/><line x1="${width/2}" x2="${width/2}" y1="${pad}" y2="${height-pad}" stroke="#d7dee8"/>${regionEllipses}${points}${targetHalos}${centers}<text x="${width-pad}" y="${height/2-5}" text-anchor="end" fill="#5f6c7b" font-size="10">PC1</text><text x="${width/2+5}" y="${pad+10}" fill="#5f6c7b" font-size="10">PC2</text></svg>`;
...
function renderExplorationCandidateTables(clusterCandidates,performanceCandidates,decisions,preferredRegionCandidates=[]) {
  ...
  const controls=item=>{const candidate=state.candidateWindows.find(window=>window.source_ref===item.candidate_id);const status=state.trainingWindows.some(window=>window.source_ref===item.candidate_id)?"accepted":"pending";const comment=candidate?.comment??commentById[item.candidate_id]??"";return `<td><input class="exploration-candidate-select" type="checkbox" data-candidate-id="${escapeHtml(item.candidate_id)}" aria-label="选择候选"></td><td>${escapeHtml(displayUiValue(status))}</td><td><input class="exploration-candidate-comment" data-candidate-id="${escapeHtml(item.candidate_id)}" value="${escapeHtml(comment)}" aria-label="候选备注"></td>`;};
...
el("stateExplorationButton").addEventListener("click", async () => {
  ...
  try {
    ...
    state.exploration=data; state.explorationPcMode="cluster"; resetExplorationRegion(); renderStateExploration(data); document.querySelector('[data-panel="stateExplorationPanel"]').click(); setStatus(`状态探索完成：${data.full_point_count} 个完整样本，返回 ${data.returned_point_count} 个显示点。候选仅供工程师比较。`,"success");
  } catch(error) { setStatus(error.message,"error"); }
  finally { setBusy(button,false,""); }
});

el("explorationPcModeCluster").addEventListener("click",()=>{ if(!state.exploration) return; state.explorationPcMode="cluster"; renderExplorationPcChart(state.exploration); });
el("explorationPcModePerformance").addEventListener("click",()=>{ if(state.exploration?.performance_config?.direction!=="target_range") return; state.explorationPcMode="performance"; renderExplorationPcChart(state.exploration); });
...
el("convertExplorationCandidates").addEventListener("click", () => {
  ...
  let added=0;
  rows.forEach(input=>{ const candidateRef=input.dataset.candidateId; const candidate=candidates.get(candidateRef); const comment=input.closest("tr")?.querySelector(".exploration-candidate-comment")?.value.trim()||""; if(candidate&&!state.candidateWindows.some(window=>window.source_ref===candidateRef)&&!state.trainingWindows.some(window=>window.source_ref===candidateRef)) { state.candidateWindows.push({id:candidateId(),start:candidate.start,end:candidate.end,source:candidate.source,source_ref:candidateRef,comment}); added+=1; } });
