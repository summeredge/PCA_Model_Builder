...
el("convertExplorationCandidates").addEventListener("click", () => {
  ...
  let added=0;
  rows.forEach(input=>{ const candidateKey=input.dataset.candidateId; const sourceRef=stateExplorationCandidateSourceRef(runId,candidateKey); const candidate=candidates.get(candidateKey); const comment=input.closest("tr")?.querySelector(".exploration-candidate-comment")?.value.trim()||""; if(candidate&&!state.candidateWindows.some(window=>window.source_ref===sourceRef)&&!state.trainingWindows.some(window=>window.source_ref===sourceRef)) { state.candidateWindows.push({id:candidateId(),start:candidate.start,end:candidate.end,source:candidate.source,source_ref:sourceRef,comment}); added+=1; } });
