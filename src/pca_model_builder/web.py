...
function renderModelVersions() {
  ...
  state.modelVersions.forEach((item,index)=>{ if(!item.integrity?.valid) return; const evidence=item.validation_evidence_status==="current"?" / 当前证据":item.validation_evidence_status==="legacy_read_only"?" / 旧证据·只读":""; const label=`${item.model_id||"legacy"}/${item.version||"legacy"} · ${item.model_status}${evidence}`; [left,right].forEach(select=>{ const option=document.createElement("option"); option.value=item.path; option.textContent=label; select.append(option); }); if(item.model_status==="validated"&&item.validation_evidence_status==="current") { const option=document.createElement("option"); option.value=item.path; option.textContent=label; publishSource.append(option); } if(index===1) right.selectedIndex=1; });
  ...
  const rows=state.modelVersions.map(item=>{ const path=String(item.path||""); const encoded=encodeURIComponent(path); const evidence=item.validation_evidence_status==="current"?" / 当前证据":item.validation_evidence_status==="legacy_read_only"?" / 旧证据·只读":""; return `<tr><td>${escapeHtml(item.model_id||"legacy")}</td><td>${escapeHtml(item.version||"legacy")}</td><td>${escapeHtml(item.model_purpose||"—")}</td><td>${escapeHtml((item.model_status||"—")+evidence)}</td><td>${item.integrity?.valid?"通过":"失败"}</td><td><a href="/download/model-version?path=${encoded}">模型包</a> <a href="/download/model-version?path=${encoded}&artifact=sha256">SHA-256</a></td></tr>`; }).join("");
